"""
generative_ai/openai.py
------------------------
A sample Generative AI application using the OpenAI API.
...
response = client.chat.completions.create(
    ...
    messages=[
        ...
        {
            "role": "user",
            "content": "When should I use RDDs over DataFrames in PySpark?"
        }
    ]
)
...
print(f"\n  Total turns in conversation: {len([m for m in conversation if m['role'] == 'user'])}")
